...
describe 'full' do
  ...
  describe 'rds' do
    ...
    it 'has a DB parameter group' do
      db_parameter_group_name =
        "database-parameter-group-#{component}-#{deployment_identifier}"
      expect(database)
        .to(have_db_parameter_group(db_parameter_group_name))
    end
    ...
    its('engine_version') { is_expected.to(eq('5.7.40')) }
