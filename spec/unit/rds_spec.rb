...
describe 'RDS' do
  ...
  describe 'by default' do
    ...
    it 'uses an engine of "mysql"' do
      expect(@plan)
        .to(include_resource_creation(type: 'aws_db_instance')
              .with_attribute_value(:engine, 'mysql'))
    end
    ...
    it 'outputs the database host' do
      expect(@plan)
        .to(include_output_creation(name: 'module_outputs')
              .with_value(including(:mysql_database_host)))
    end

    it 'outputs the database name' do
      expect(@plan)
        .to(include_output_creation(name: 'module_outputs')
              .with_value(including(:mysql_database_name)))
    end

    it 'outputs the database port' do
      expect(@plan)
        .to(include_output_creation(name: 'module_outputs')
              .with_value(including(:mysql_database_port)))
    end
  end
  ...
  describe 'when database_port provided' do
    ...
    it 'listens on the provided port' do
      expect(@plan)
        .to(include_resource_creation(type: 'aws_db_instance')
              .with_attribute_value(:port, 3307))
    end
  end
